...
public class CinemaReservationProvider {
    ...
    private boolean checkAvailabilityOfPlaces(final boolean[][] allCinemaPlacesMatrix, int rowNum, int numPlaces) {
        ...
        startingPosition = testedSTRING.indexOf(targetSTRING); //we search for the first position (if ever) of numPlaces consecutive free seats
    ...
    private void displayNegativeReservationMessage(final int rowNum, final int numPlaces) {
        System.out.println();
        System.out.println("---------------------------------------------------------------------------------");
        System.out.println(String.format("SORRY!! Could not find %d consecutive places on row %d. Please try again...", numPlaces, rowNum));
        System.out.println();
    }

    private void displayReservedPlacesLayout(final boolean[][] allPlaces, int rowReserved, int startingPosition, int numSeatsRequested) {
